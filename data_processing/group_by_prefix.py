"""
1. Groups contracts/ PDFs by shared 5-digit prefix.
2. Prints groups sorted by total size, smallest first, stopping once the
   cumulative file count across listed groups reaches or exceeds 100.
...
selected_dir = "selected_contracts"
output_csv   = "selected_contracts.csv"

# Step 1: unique 5-digit prefixes from filenames that start with 5 digits
ac_files = [f for f in os.listdir(all_contract) if os.path.isfile(os.path.join(all_contract, f))]
...
def group_size(file_list):
    return sum(os.path.getsize(os.path.join(all_contract, f)) for f in file_list)
...
# Step 5: print until cumulative file count >= 100 (finish current group)
...
for p, files, size in ranked:
    ...
    if cumul >= 130:
        print(f"\nStopped: {cumul} cumulative files across {len(included) and ranked.index((p, files, size)) + 1} groups.")
        break
...
# Step 6: copy selected PDFs into selected_contracts/
if os.path.exists(selected_dir):
    shutil.rmtree(selected_dir)
os.makedirs(selected_dir)
for f in included:
    shutil.copy2(os.path.join(all_contract, f), os.path.join(selected_dir, f))
print(f"\nCopied {len(included)} PDFs → {selected_dir}/")

# Step 7: write full_executed.csv with .pdf filenames for extract_contracts.py
